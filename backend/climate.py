...
async def _fetch_met_norway_forecast(
    ...
    try:
        ...
        logger.warning("MET Norway transport failure: %s", type(exc).__name__)
        raise HTTPException(
            status_code=502,
            detail="Climate forecast request failed.",
        ) from exc
    ...
    for day in sorted(grouped)[:forecast_days]:
        ...
        rows.append({
            ...
            "precipitation_mm": (
                round(sum(precipitation), 2) if precipitation else 0.0
            ),
            "precipitation_probability_percent": (
                round(max(rain_prob), 2) if rain_prob else None
            ),
            "wind_max_kmh": (
                round(max(wind), 2) if wind else None
            ),
            "et0_mm": None,
    ...
    return {
        ...
        "daily": {
            ...
            "temperature_2m_mean": [
                row["temperature_mean_c"] for row in rows
            ],
            "temperature_2m_max": [
                row["temperature_max_c"] for row in rows
            ],
            "temperature_2m_min": [
                row["temperature_min_c"] for row in rows
            ],
            "precipitation_sum": [
            ...
            "wind_speed_10m_max": [
                row["wind_max_kmh"] for row in rows
            ],
            "et0_fao_evapotranspiration": [
                row["et0_mm"] for row in rows
            ],
            "weather_code": [
                row["weather_code"] for row in rows
            ],
        },
    }
...
async def climate_intelligence(request: ClimateRequest):
    ...
    try:
        ...
        if response.status_code == 429:
            logger.warning(
                "Open-Meteo quota exhausted; using MET Norway fallback."
            )
            ...
            if response.status_code != 200:
                ...
                response = await _fetch_forecast(
                    client,
                    fallback_params,
                )
        ...
        logger.warning(
            "Climate forecast transport failure: %s",
            type(exc).__name__,
        )
        raise HTTPException(
            status_code=502,
        ...
        logger.exception(
            "Climate forecast unexpected failure: %s",
            type(exc).__name__,
        )
        raise HTTPException(
            status_code=502,
    ...
    for i, day in enumerate(times):
        ...
        rows.append({
            ...
            "wind_max_kmh": get_value("wind_speed_10m_max"),
    ...
    if max_temp is not None and max_temp >= 35:
        signals.append({
            ...
            "evidence": (
                f"Forecast maximum temperature reaches "
                f"{max_temp:.1f} °C."
            ),
            "threshold_note": (
                "35 °C is a screening threshold, not a "
                "crop-specific damage threshold."
            ),
        })
    ...
    if max_rain_prob is not None and max_rain_prob >= 70:
        signals.append({
            ...
            "status": "elevated",
            "evidence": (
                f"At least one forecast day has "
                f"{max_rain_prob:.0f}% maximum precipitation probability."
            ),
            "threshold_note": (
                "This is a forecast probability, not a measured "
                "rainfall amount."
            ),
        })
    ...
    if (
        total_et0 is not None
        and total_rain is not None
        and total_et0 > total_rain
    ):
        signals.append({
            "type": "atmospheric_water_demand",
            "status": "et0_above_forecast_rain",
            "evidence": (
                f"Forecast reference ET0 totals {total_et0:.1f} mm "
                f"versus {total_rain:.1f} mm precipitation."
            ),
            "limitation": (
                "ET0 is reference evapotranspiration, not crop water "
                "use or irrigation requirement."
            ),
        })
    ...
    if max_wind is not None and max_wind >= 30:
        signals.append({
            ...
            "status": "elevated",
            "evidence": (
                f"Forecast maximum wind speed reaches "
                f"{max_wind:.1f} km/h."
            ),
            "threshold_note": (
                "30 km/h is a screening threshold, not a "
                "crop-specific damage threshold."
            ),
        })
