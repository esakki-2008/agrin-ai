...
class _FarmDigitalTwinPageState extends State<FarmDigitalTwinPage>{
  ...
  String _satelliteSummary(dynamic raw){
    ...
    if(raw['cloud_cover_percent'] is num) {
      final cloud=(raw['cloud_cover_percent'] as num).toStringAsFixed(2);
      parts.add('Cloud $cloud%');
    }
    if(indices['ndvi'] is num) {
      final ndvi=(indices['ndvi'] as num).toStringAsFixed(3);
      parts.add('NDVI $ndvi');
    }
    if(indices['ndmi'] is num) {
      final ndmi=(indices['ndmi'] as num).toStringAsFixed(3);
      parts.add('NDMI $ndmi');
    }
    if(indices['evi'] is num) {
      final evi=(indices['evi'] as num).toStringAsFixed(3);
      parts.add('EVI $evi');
    }
    return parts.isEmpty ? 'Available' : parts.join(' • ');
  }
